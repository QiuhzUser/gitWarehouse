        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="supSaleAttr in spuSaleAttrList" :key="supSaleAttr.id">
                <dt class="title">{{ supSaleAttr.saleAttrName }}</dt>
                <dd
                  changepirce="40"
                  :class="{ active: supSaleAttrValue.isChecked == 1 }"
                  v-for="supSaleAttrValue in supSaleAttr.spuSaleAttrValueList"
                  :key="supSaleAttrValue.id"
                  @click="
                    changeActive(
                      supSaleAttrValue,
                      supSaleAttr.spuSaleAttrValueList
                    )
                  "
                >
                  {{ supSaleAttrValue.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前路由跳转是:从a到b,这里加入购物车,进行跳转之前,还发请求,把购买的产品信息通过请求的形式通知服务器,服务器进行存储 -->
...
export default {
  ...
  computed: {
    ...
    //给子组件的数据
    skuImageList() {
      //如果服务器没有数据回来skuInfo这个对象是个undefined
      return this.skuInfo.skuImageList || [];
    },
  },
  methods: {
    changeActive(saleAttrValue, arr) {
      //遍历全部售卖的属性值isChecked为零  无高亮
      arr.forEach((item) => {
        item.isChecked = "0";
      });
      //点击的那个售卖属性为高亮
    ...
    async addShopcar() {
      ...
      //3：失败，给用户提示
      //判断加入购物车是否成功
      //调用仓库中的方法，这个方法因为有async,返回的是个promise，要么成功||要么失败
      try {
