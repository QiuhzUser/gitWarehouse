...
export default {
  ...
  methods: {
    ...
    //非正常情况(用户操作过快):本身全部的一级分类都应该触发鼠标进入事件，结果只有部分被触发
    ...
    goSearch(event) {
      //最好的解决方案：编程式导航+事件委派
      //1.存在的问题：事件委派，是把全部子节点【h3、dt、dl、em、】的事件委派给父亲节点
      //点击a标签的时候，才会进行路由跳转【怎么确定点击的一定是a标签】
      //2.存在的另一个问题：即使能确定点击的是a标签，如何区分是一级、二级、三级的a标签
      //第一个问题：把子节点当中a标签加上自定义属性data-categoryName，其余的子节点没有
      let element = event.target;
      //获取到当前触发这个事件的节点，需要带有data-categoryName这样节点，一定是a标签
      //节点有一个属性dataset属性，可以获取节点的自定义属性与属性值
      // console.log(element.dataset);
      //element.dataset 是个对象利用解构赋值直接写成以下简写
